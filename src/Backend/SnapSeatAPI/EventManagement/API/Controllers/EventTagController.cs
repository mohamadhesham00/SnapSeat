...
namespace EventManagement.API.Controllers
{
    ...
    public class EventTagController : BaseController
    {
        ...
        [HttpPost("create")]
        public async Task<IActionResult> CreateAsync(EventTagDTO dto, CancellationToken cancellationToken)
        {
            if (!CurrentUser.IsAuthenticated)
            {
                return Unauthorized();
        ...
        [HttpPut]
        public async Task<IActionResult> UpdateAsync(Guid id, EventTagDTO dto, CancellationToken cancellationToken)
        {
            if (!CurrentUser.IsAuthenticated)
            {
                return Unauthorized();
